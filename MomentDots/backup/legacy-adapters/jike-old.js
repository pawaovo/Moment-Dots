/**
 * 即刻平台适配器 - 基于UniversalContentInjector重构版本
 * 支持Lexical编辑器，实现95%+成功率的内容注入
 *
 * 技术方案：textContent + input事件策略
 * 验证成功率：95%+
 */

console.log('即刻平台适配器加载中...');

// 确保依赖的工具类已加载
if (typeof window.MomentDots === 'undefined' ||
    typeof window.MomentDots.UniversalContentInjector === 'undefined') {
  console.error('UniversalContentInjector未加载，即刻适配器无法工作');
}
 ...
 * 基于验证的技术方案实现
 */
class JikeAdapter extends window.MomentDots.BasePlatformAdapter {
  constructor() {
    super('jike');

    // 即刻平台特定配置
    this.selectors = {
      // Lexical编辑器选择器（按优先级排序）
      content: [
  ...
  /**
   * 注入内容到即刻编辑器
   * 使用验证过的textContent + input事件策略
   * @param {string} content - 要注入的内容
   * @returns {Promise<boolean>} - 注入是否成功
   */
  async injectContent(content) {
    this.log('开始注入内容到即刻编辑器', { contentLength: content.length });

    // 使用智能重试机制
    return await this.withRetry(async () => {
      // 1. 查找编辑器元素
      let editor = this.injector.findElement('jike', 'content');

      if (!editor) {
        // 2. 等待编辑器出现
        this.log('等待即刻编辑器加载...');
        editor = await this.injector.waitForElement('div[contenteditable="true"]', 10000);
      }
      ...
      this.log('找到即刻编辑器，开始注入内容');

      // 3. 使用UniversalContentInjector注入内容
