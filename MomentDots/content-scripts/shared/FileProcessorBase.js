...
class FileProcessorBase {
  ...
  async getFileWithChunking(fileId) {
    try {
      // 首先检查文件是否需要分块传输
      const chunkCheckResponse = await new Promise((resolve, reject) => {
      ...
      if (!chunkCheckResponse.success) {
        throw new Error(chunkCheckResponse.error || 'Failed to check file chunking');
      }

      // 如果不需要分块，使用原有方法
      if (!chunkCheckResponse.needsChunking) {
        this.log(`文件 ${fileId} 无需分块传输 (${chunkCheckResponse.fileSize} bytes)`);
        return await this.getFileFromExtension(fileId);
      }

      // 需要分块传输
      this.log(`文件 ${fileId} 需要分块传输 (${chunkCheckResponse.fileSize} bytes)`);
      return await this.getFileWithChunkTransfer(fileId);
   ...
   * @returns {Promise<File>} File对象
   */
  async getFileWithChunkTransfer(fileId) {
    try {
      // 获取文件分块信息
      const chunkInfoResponse = await new Promise((resolve, reject) => {
        chrome.runtime.sendMessage({
          action: 'getFileChunkInfo',
          fileId: fileId
        }, (response) => {
          if (chrome.runtime.lastError) {
            reject(new Error(chrome.runtime.lastError.message));
      ...
      if (!chunkInfoResponse.success) {
        throw new Error(chunkInfoResponse.error || 'Failed to get chunk info');
      }

      const { chunkInfo } = chunkInfoResponse;
      this.log(`开始分块传输: ${chunkInfo.fileName}, 总块数: ${chunkInfo.chunkInfo.totalChunks}`);

      // 获取所有分块
      const chunks = [];
      for (let i = 0; i < chunkInfo.chunkInfo.totalChunks; i++) {
        ...
        if (!chunkResponse.success) {
          throw new Error(`Failed to get chunk ${i}: ${chunkResponse.error}`);
        }

        // 转换数据格式
        const chunkData = chunkResponse.chunkData;
        chunks.push({
        ...
        this.log(`获取分块 ${i + 1}/${chunkInfo.chunkInfo.totalChunks} 完成`);
      }
      ...
      const reconstructedFile = await this.reconstructFileFromChunks(chunks, metadata);
      this.log(`文件重组完成: ${reconstructedFile.name} (${reconstructedFile.size} bytes)`);

      return reconstructedFile;

    } catch (error) {
      this.logError('分块传输失败', error);
      throw error;
    }
  }
   ...
   */
  async reconstructFileFromChunks(chunks, metadata) {
    // 验证分块完整性
